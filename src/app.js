...
function splitter(conf_directory) {
    ...
    var server = net.createServer(function (localSocket) {
        ...
        localSocket.on('data', function (data) {
            ...
            console.log(`splitter received data: ${data.slice(0, 30)} ... ${data.slice(-30)}`);
            ...
            if (idx == -1) {
                part_1 = data;
                console.log(`no \\n in data, send data to target ${host}:${port}`);
                writeToSocket(part_1, outSocks[sockIdx], localSocket);
                console.log(`data is: ${data.slice(0, 30)} ... ${data.slice(-30)}`);
            }
            else {
                part_1 = data.slice(0, idx + 1); /* include the line termination */
                part_2 = data.slice(idx + 1);

                console.log(`send to socket[${sockIdx}]: part_1`);
                console.log(`data is: ${part_1.slice(0, 30)} ... ${part_1.slice(-30)}`);
                writeToSocket(part_1, outSocks[sockIdx], localSocket);
                sockIdx++;
                sockIdx %= outSocks.length;

                console.log(`send to socket[${sockIdx}]: part_2`);
                console.log(`data is: ${part_2.slice(0, 30)} ... ${part_2.slice(-30)}`);
...
function target(conf_directory) {
    ...
    var server = net.createServer(function (localSocket) {
        ...
        localSocket.on('data', function (data) {
            ...
            console.log(`received data: ${data.slice(0, 30)} ... ${data.slice(-30)}`);

            fs.appendFile(outputfile, data, function () {
                // written to file
                // console.debug(`Written to file: ${data}`);
                console.debug(`Written to file: ${data.slice(0, 30)} ...${data.slice(-30)}`);
